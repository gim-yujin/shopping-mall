...
> **주의**: 아래 표의 COUNT 실행시간 `~23ms`는 본 변경 설계 당시의 EXPLAIN ANALYZE 추정값(구환경 가정)이다. 2026-04-21 PG 16.13·50K 활성 상품에서 재측정하면 `Index Only Scan using idx_product_review_count`가 선택되어 약 **7.7ms**에 완료됨을 §2-3에서 확인했다. 아래 절대 수치는 설계 결정의 근거로 남기되, 현재 스케일의 실측 수치는 §4-3·§10-7과 같이 별도로 기록한다.

| 항목 | 기존 | 개선 후 | 근거 |
|:-----|:-----|:--------|:-----|
| 상품 목록 캐시 미스당 COUNT 실행 횟수 | 조합 수만큼 (~25회/10분) | 1회/10분 | 캐시 키 설계 |
| COUNT 쿼리 1회 실행 시간 (설계 시 가정) | ~23ms | ~23ms (변화 없음) | EXPLAIN ANALYZE (구환경) |
| 캐시 미스 시 단일 요청의 총 DB 시간 | content(~0.5ms) + count(~23ms) = **~23.5ms** | content(~0.5ms) + count-cache-hit(≈0ms) = **~0.5ms** | 로컬 측정 |

캐시 미스 storm 상황(`productList` TTL 만료 직후 25개 조합 동시 미스)에서 **총 DB 시간 ~575ms → ~23ms**로 감소할 것으로 설계 시 예상했다. PG 16.13 실측(7.7ms)을 적용하면 같은 상황에서 ~192ms → ~8ms 수준이지만, 핵심은 절대 시간이 아니라 HikariCP 풀(17) 포화 구간을 축소해 p95 꼬리를 단축한다는 방향성이며, §4-3 격리 실험(p95 −11%, p99 −17%)에서 그 효과가 실측 확인되었다.
