# 회고/아카이브: ShopMall 프로젝트 Phase 0 보고서
...
프로젝트 시작 전 성능 최적화를 고려하여 직접 설계한 PostgreSQL 스키마로, `schema.sql` 기준 17개 테이블과 52개 인덱스(일반 50개 + UNIQUE 2개)로 구성되어 있다. 각 테이블에는 예상 레코드 수를 주석으로 명시하여 대규모 환경을 전제로 설계했다.
...
후속 기능 도입으로 추가된 테이블의 목적은 다음과 같다.
- `review_helpfuls`: 리뷰에 대한 사용자별 “도움이 돼요” 반응을 저장하고, 중복 반응을 방지하기 위한 이력/정합성 관리용 테이블
- `point_history`: 포인트 적립/사용/취소/만료 내역을 참조 키와 함께 남겨 포인트 정산 추적성을 확보하는 원장성 이력 테이블
...
총 52개 인덱스(`CREATE INDEX` 50개 + `CREATE UNIQUE INDEX` 2개)를 설계했으며, 크게 5가지 유형으로 분류된다.
...
### 4.1 주문 생성 (Order Creation) — 조건부 다중 테이블 원자적 변경

이 프로젝트에서 가장 복잡한 비즈니스 로직은 주문 생성이다. `POST /orders` 하나의 HTTP 요청이 단일 `@Transactional` 안에서 **기본 5개 + 조건부 2개 이상의 테이블**을 원자적으로 변경한다.
...
[OrderController]  userId 추출 → orderService.createOrder() 호출
       │
       ▼
[OrderCreationService @Transactional]  ← 여기서부터 단일 트랜잭션
       │
       ├─ Step 0: 사용자 + 등급 + 장바구니 락/대상 조회
       │   cartRepository.acquireUserCartLock(userId)  ← 사용자 장바구니 직렬화
       │   if cartItemIds 존재: findByUserIdAndCartIdIn(...)  ← 부분 주문
       │   else: findByUserIdWithProduct(userId)               ← 전체 주문
       ...
       │   inventory snapshot 임시 보관 (orderId는 save 후 연결)
       │
       ...
       │   earnedPointsSnapshot = finalAmount × tier.pointEarnRate / 100
       │   order.markPaid()
       ...
       │
       ├─ Step 6: 포인트 사용 처리 + 누적금액 갱신
       │   user.addTotalSpent(finalAmount)     ← [users] UPDATE
       │   포인트 사용 시 user.usePoints(usePoints) + point_history(USE) 기록
       │   적립 포인트는 earnedPointsSnapshot으로 주문에 저장만 하고 즉시 적립하지 않음
       │
       ├─ Step 7: 등급 재산정
       ...
       │
       └─ Step 8: 장바구니 삭제
           if 부분 주문: deleteAllById(선택 항목)     ← [carts] DELETE (부분)
           else 전체 주문: deleteByUserId(userId)     ← [carts] DELETE (전체)
       │
       ▼
[Hibernate Flush]  Dirty Checking → 조건에 따라 5~8개 테이블 UPDATE/INSERT/DELETE
       │
       ▼
[COMMIT]  성공 시 변경 대상 테이블 동시 반영 / 실패 시 전체 ROLLBACK
       │
       ▼
[302 Redirect → GET /orders/{id}]  PRG 패턴
```

**주문 생성 시 변경 대상 테이블(조건부):**

| 순서 | 테이블 | 기본/조건 | 변경 유형 | 실행 조건/내용 |
|------|--------|----------|----------|----------------|
| 1 | `products` | 기본 | UPDATE | stock_quantity 감소, sales_count 증가 |
| 2 | `orders` | 기본 | INSERT | 주문 헤더 (포인트 적립 예정치 snapshot 포함) |
| 3 | `order_items` | 기본 | INSERT | 주문 상세 (cascade) |
| 4 | `users` | 기본 | UPDATE | total_spent 반영, usedPoints가 있으면 point_balance 차감, tier_id 갱신 가능 |
| 5 | `carts` | 기본 | DELETE | 부분 주문이면 선택 항목만 삭제, 전체 주문이면 사용자 장바구니 전체 삭제 |
| 6 | `product_inventory_history` | 기본 | INSERT | 재고 변동 이력 OUT (order save 후 orderId 연결) |
| 7 | `user_coupons` | 조건 | UPDATE | 쿠폰을 실제 적용한 경우에만 is_used=true, used_at, order_id |
| 8 | `point_history` | 조건 | INSERT | 포인트 사용(`usePoints > 0`) 또는 추후 적립/환불 이벤트 발생 시 기록 |

> 정리: 문서상 "7개 테이블"은 대표 케이스이며, 실제 코드는 **쿠폰 사용 여부 / 포인트 사용 여부 / point_history 기록 여부**에 따라 변경 테이블 수가 달라진다.
...
```
cancelOrder(orderId)
  ├─ cancellable 상태 검증 (PENDING/PAID)
  ├─ order.items를 productId 순 정렬 후 재고 복구(remainingQuantity 기준)
  │   ├─ product.increaseStockAndRollbackSales(...)  → 재고/판매량 복구
  │   └─ inventoryHistory(change_type='IN')           → 재고 변동 이력(입고)
  ├─ user.addTotalSpent(남은 환불액.negate())         → 누적금액 차감
  ├─ usedPoints 미환불분만 환불 + point_history(REFUND) 기록
  ├─ userCoupon.cancelUse()                           → 쿠폰 복구 (주문 연계 해제)
  ├─ tierRepository → user.updateTier()               → 등급 재산정 (강등 가능)
  └─ order.cancel()                                   → 주문 상태 CANCELLED
```

주문 생성 시 적립 포인트는 즉시 지급하지 않고 snapshot만 저장하므로, 취소 시에는 `earnedPoints` 회수가 아니라 **사용 포인트 환불**만 처리하면 된다. 또한 부분 취소가 선행된 경우 `refundedAmount/refundedPoints`를 고려해 잔여분만 보상한다.
...
4. 포인트 적립: `earnedPointsSnapshot`으로 저장 후 **배송 완료(DELIVERED) 시점**에 실제 적립 (`settleEarnedPoints()`)
...
**해결 (4개 파일):**
- `OrderCancellationService.cancelOrderInternal()`: `remainingRefundAmount` 차감, `usedPoints` 미환불분 환불, 쿠폰 `cancelUse()` 복원 로직 추가
...
- `@Transactional` 단일 트랜잭션 내에서 기본/조건 분기를 포함한 다중 테이블 원자적 변경 로직을 안전하게 처리했다.
